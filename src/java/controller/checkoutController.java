    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            if (info == null) { 
                ...
                order.setDate((new Timestamp(System.currentTimeMillis())).toString());
                dao.updateOrder(order);
                if (request.getParameter("method").equals("cash"))
                    dao.checkOutOrder(order_id, 0);
                else 
                    dao.checkOutOrder(order_id, -1);
